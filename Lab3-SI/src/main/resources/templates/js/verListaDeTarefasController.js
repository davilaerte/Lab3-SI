...
app.controller("verListaDeTarefasController", function($scope, $http, $location) {
	...
	$scope.carregarListasDeTarefas = function () {
		$http.get("http://" + location.host + "/ListaDeTarefa/ListaDeTarefaReduzida").success(function (data, status){
	...
	$scope.deletarListaDeTarefa = function(listaDeTarefa) {
		...
		$http.delete("http://" + location.host + "/ListaDeTarefa/" + listaDeTarefa.id).success(function (data, status){
	...
	$scope.removerTodasAsTarefasDasListas = function() {
		$http.delete("http://" + location.host + "/ListaDeTarefa/allTarefa").success(function (data, status) {
